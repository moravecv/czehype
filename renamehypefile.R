...
renamehypefile <- function(in_file, name_table, col_old, col_new){
  ...
  for (i in colnames(in_file)[2:ncol(in_file)]){ # for each column name
    a <- name_table[get(col_old) == i, get(col_new)] # find new name in name table 
    b <- which(colnames(in_file) == i) # find a position of old column name in input file
    colnames(in_file)[b] <- a # replace the old column name with a new one
  }
